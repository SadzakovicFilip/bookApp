...
function BookList({bookList,setBookList}) {
    ...
    const singleBook=bookList?.map((item,key)=>{
        return (<div className="singleBook" key={key}> 
        <h1>{key+1}.</h1>
        <div className='title'>Naslov:<b>{item.book}</b></div>
        <div className='author'>Autor:<b>{item.author}</b></div>
        <div className='pages'>Broj strana:<b>{item.pages}</b></div>
